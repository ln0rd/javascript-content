...
const taskName = 'TAKE_MONEY_FROM_WALLET_TASK'
const Logger = createLogger({ name: taskName })

export default class TakeMoneyFromWallet {
  static type() {
    return 'manual'
  }
  ...
  static handler(paramArray) {
    ...
    return client
      .takeMoneyFromWallet(params.walletId, {
        amount: params.amount,
        requestId: params.uid
      })
      .then(() => {
        Logger.info({ args }, 'takeMoneyFromWalletTaskSuccess')
      ...
      .catch(err => {
        Logger.error({ err, args }, 'takeMoneyFromWalletTaskError')
        return ManualTaskHistory.create({
          task: taskName,
          status: FAILED,
          args
