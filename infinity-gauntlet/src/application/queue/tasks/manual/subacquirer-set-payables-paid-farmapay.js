...
export default class SubacquirerSetPayablesPaidFarmapay {
  static type() {
    return 'manual'
  }
  ...
  static handler(args) {
    ...
    function setPayablesPaid(provider) {
      ...
      return Promise.resolve()
        .then(findFarmaPayPayables)
        .tap(checkPayables)
        .each(updateSettlementAndPayables)
        .then(finishProvider)

      async function findFarmaPayPayables() {
        const payables = await Payable.find({
          company_id: '606382a65b80110006d7f709', // Farma Pay
        ...
        if (!payables || !payables.length) {
          throw Error('Failed to find Farmapay Payables')
        }

        const fakeAggregate = []

        const farmapayAffiliationId = payables[0].affiliation_id

        fakeAggregate.push({
          _id: farmapayAffiliationId,
          payables: payables
        })
