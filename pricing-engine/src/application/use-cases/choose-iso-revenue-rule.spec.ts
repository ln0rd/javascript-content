...
describe('#chooseIsoRevenueRule', () => {
  ...
  describe('when a single rule is applicable', () => {
    let dbRules: IsoRevenueRule[]
    let result: Nullable<IsoRevenueRule>
    ...
    beforeAll(async () => {
      ...
      ].map((r) => IsoRevenueRulesRepository.fromDB(r))

      sinon
        .stub(IsoRevenueRulesRepository, 'findActiveRulesByTarget')
        .resolves(dbRules)

      const revenueRule = new ChooseIsoRevenueRule()
    ...
    test('returns an IsoRevenueRule', () => {
      expect(result).toBeInstanceOf(IsoRevenueRule)
    })

    test('returns an IsoRevenueRule with a flat or a percentage value', () => {
      expect(!!result?.flat || !!result?.percentage).toEqual(true)
    })
  })

  describe('when more than one rule is applicable', () => {
    let dbRulesSortedByDate: IsoRevenueRule[]
    let result: Nullable<IsoRevenueRule>
    ...
    describe('with one matching iso rule and one matching merchant rule', () => {
      ...
      test('returns the most specific rule for the target', () => {
        expect(result?.id).toEqual('4969b255-d742-4e57-9cfa-715466bcf816')
      })
    })
  })
